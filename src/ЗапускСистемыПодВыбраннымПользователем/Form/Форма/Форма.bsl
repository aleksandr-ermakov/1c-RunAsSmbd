...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТаймаутПодменыПароля = Элементы.ТаймаутПодменыПароля.МинимальноеЗначение;
	...
	ИспользоватьОкноТаймера = Истина;
...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)    
	ТаймаутПодменыПароля = Макс(ТаймаутПодменыПароля, Элементы.ТаймаутПодменыПароля.МинимальноеЗначение); 
...
Процедура ПодменитьПарольВременно(Команда)
	...
	Если ИспользоватьОкноТаймера Тогда
		ЗапуститьТаймерВОтдельномОкне();
	Иначе
...
Функция ПараметрыКоманднойСтрокиДляЗапуска()
	...
	// Вариант интерфейса
	Если ВариантИнтерфейса = "ИнтерфейсТакси" Тогда
		ПараметрыКоманднойСтроки.Добавить("/itdi");
	КонецЕсли;
	Если ВариантИнтерфейса = "Интерфейс85" Тогда
		ПараметрыКоманднойСтроки.Добавить("/i85");
	КонецЕсли;
...
&НаКлиенте
Процедура ЗапуститьТаймерВОтдельномОкне()

	ДополнительныеПараметры = Новый Структура;
	ОписаниеОповещения = Новый ОписаниеОповещения("ПриЗакрытииОкнаТаймера", ЭтаФорма, ДополнительныеПараметры);
	ЧастиИмениФормы = СтрРазделить(ИмяФормы, ".");
	ИмяФормыТаймера = СтрШаблон("%1.%2.Форма.ОбратныйОтсчет", ЧастиИмениФормы[0], ЧастиИмениФормы[1]);
	ПараметрыФормы = Новый Структура("КоличествоСекунд", ТаймаутПодменыПароля);
	ОткрытьФорму(ИмяФормыТаймера, ПараметрыФормы, ЭтаФорма, , , , ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);

КонецПроцедуры // ЗапуститьТаймерВОтдельномОкне() 

// При закрытии окна таймера, восстанавливает пароль.
// 
// Параметры:
//	Результат				 - Произвольный - Значение результата, переданное вторым параметром при вызове метода.
//	ДополнительныеПараметры	 - Произвольный - Значение, которое было указано при создании объекта ОписаниеОповещения.
//
&НаКлиенте
Процедура ПриЗакрытииОкнаТаймера(Результат, ДополнительныеПараметры) Экспорт

	ВосстановитьПарольНаКлиенте(); 
	
КонецПроцедуры // ПриЗакрытииОкнаТаймера()
